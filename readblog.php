...
<body>
 ...
 foreach ($json as $k ) {
   if($k['bid']==$_GET['bid']){
    $a=$_GET['bid'];
    $stmt = $pdo->query("SELECT u.uid,title,star,total,uname,date FROM blog b join users u on b.uid=u.uid where bid=$a;");
$rows = $stmt->fetch(PDO::FETCH_ASSOC);/*
if($rows['approve']==1 || $_SESSION['role']==1){
    if(isset($_POST['submit'])&& isset($_POST['satisfied']) && $_POST['satisfied']>0)
    {
      $c=$_POST['satisfied']+$rows['star'];
      $p=$rows['total']+1;
             $stmt1 = $pdo->prepare('UPDATE   blog set star=:t , total=:u where bid=:a');
        $stmt1->execute(array(
                ':t' => $c,
                ':u' => $p,
                ':a'=>$a)
        );
        $stmt = $pdo->prepare('INSERT INTO rate (bid,uid) VALUES ( :a,:c)');
                ...
                $stmt = $pdo->prepare('INSERT INTO review (view,uid,uname,date,bid) VALUES ( :a,:c,:b,:d,:e)');
                ...
                header("Location: readblog.php?bid=".$bi);
                return;
    }*/
    $date2 = strtotime($rows['date']);
    echo '<section class="text-gray-600 body-font">
  <div class="container px-5 py-24 mx-auto">
    <div class="flex flex-col text-center w-full mb-12">
      <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">'.$rows['title'].'</h1>
      <h3 class="sm:text-xl text-xl font-medium title-font mb-4 text-gray-400">'.$rows['uname'].'</h3>
      <h3 class="sm:text-xl text-xl font-medium title-font mb-4 text-gray-400">'.date('j F Y', $date2).'</h3>
      <br><br>
      <p class="lg:w-2/3 mx-auto leading-relaxed text-base">'.$k['text'].'</p>
    </div>
  </div>
</section>';
   ...
   /* if($rows['star']==0){echo "No ratings yet";}
    else{  $rate=$rows['star']/$rows['total'];
            $t=5-$rate;$i=1;
    while ($i<=5) {
              if($i<=$rate)echo "<span class='fa fa-star checked'></span>";
              else echo "<span class='fa fa-star'></span>";
              $i=$i+1;
            }}
            echo"</div>";
            if(isset($_SESSION['uid']) && $_SESSION['uid']!=$rows['uid'] && $fl!=1){
          echo '<h2 class="head2" style="display:inline;">Please Rate here:</h2>
              <form method="post" class="box" style="margin-top:10px;width:100%;">
                <label class="r">
...
';}*/
            if(isset($_SESSION['uid']) && $_SESSION['uid']!=$rows['uid']){
             echo '<br>

             <textarea id="subject" name="review" class="form-control mt-4" style="width:100%;" rows="5" placeholder="Write a review"></textarea>
...
<script>
  ...
  $(".r").click(function(){
    ...
    $(this).prevAll().find("i").addClass('checked');
     $(this).nextAll().find("i").removeClass('checked');
  });
});
</script>
</body>
