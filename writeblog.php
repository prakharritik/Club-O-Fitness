...
if (!isset($_SESSION['name'])) {
    ...
    if (strlen($_POST['title']) < 1 || strlen($_POST['image']) < 1) {
        $_SESSION['error'] = 'All values are required';
        header("Location: writeblog.php");
        return;
    } else {
            
        $stmt = $pdo->prepare('INSERT INTO  blog( title,uid,date,image,des) VALUES ( :t, :uid,:d,:im ,:de)');
        $stmt->execute(array(
                ':t' => $_POST['title'],
                ':uid' => $_SESSION['uid'],
                ':d'=>date("Y-m-d"),
...
<?php if(isset($_SESSION['error']))echo $_SESSION['error'];?>
  <form id="someForm" class="box" method="post">
  ...
  <div class="container px-5 py-24 mx-auto">
    ...
    <div class="lg:w-1/2 md:w-2/3 mx-auto">
      <div class="flex flex-wrap -m-2">
        ...
        <div class="p-2 w-full">
          <div class="relative">
            <label for="name" class="leading-7 text-sm text-gray-600">Cover Image URL</label>
            <input type="text"  name="image" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          </div>
        </div>
        <div class="p-2 w-full">
          <div class="relative">
            <label for="name" class="leading-7 text-sm text-gray-600">One line description</label>
            <input type="text"  name="des" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          </div>
        </div>
      ...
      <div id="hiddenFormWrap">
        <input type="hidden" name="divData" id="useDataField" /></div>
    </form>
    <form id="get-data-form" method="post">

      <textarea class="tinymce" id="texteditor"></textarea>
      <div class="p-2 w-full">
          <button type="submit" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">Preview</button>
        </div>
      ...
      <button type="submit"  id="saveBtn" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">Submit</button>
