...
$stmt = $pdo->prepare('SELECT * FROM issues where uid   = :ii ');
    $stmt->execute(array(":ii" => $_SESSION['uid']));
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
     $stmt = $pdo->prepare('SELECT * FROM issues i join prescription p on i.iid=p.pid where uid   = :ii order by i.iid desc');
    $stmt->execute(array(":ii" => $_SESSION['uid']));
...
if ( isset($_POST['review'])  ) {
          ...
          $fileName=basename($_FILES['file']['name']);
            ...
            $folder_path='images/';
            $newImage_name=$folder_path.$_SESSION['name'].date("Y-m-d_G-i-s") ."_".$fileName;
            if(!move_uploaded_file($_FILES["file"]["tmp_name"], $newImage_name)) 
            {
              Print "<script>alert('Problem could not move file to destination. Please check again.')</script>";
              header("Location:postproblem.php");
              return;
            }
            $stmt = $pdo->prepare('INSERT INTO issues (name, problem, uid,file) VALUES (  :fn, :ln, :em,:mp)');
  ...
  $stmt = $pdo->prepare('INSERT INTO issues (name, problem, uid) VALUES (  :fn, :ln, :em)');
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js" integrity="sha512-894YE6QWD5I59HgZOGReFYm4dnWc1Qt5NtvYSaNcOP+u1T9qYdvdihz0PPSiiqn/+/3e7Jo4EaG7TubfWGUrMQ==" crossorigin="anonymous"></script><link href="https://unpkg.com/tailwindcss@^2.0.4/dist/tailwind.min.css" rel="stylesheet">
...
<body>
  ...
  <nav class="flex items-center justify-between flex-wrap bg-teal p-6">
	...
	<h1 class="text-center">Post a problem</h1>
	<p class="text-center">Have some health issue? Write to us. Our health expert will respond to your problem soon. You can also send a picture in case of some skin problem/ issue which requires visual.</p>
                       

                     <div >
                       <form method="POST" id="fo" enctype="multipart/form-data">
                <input type="file" name="file" id="form2"  accept="image/*">
<i class="fas fa-paperclip" style="font-size: 30px;float: right;margin-right: 20px;display: inline;"></i>       
                                 <textarea id="subject" name="review" class="form-control" style="width:100%;background-color: #8ae9b3;
background-image: linear-gradient(315deg, #8ae9b3 0%, #c8d6e5 74%);margin-top: 30px;" rows="5" placeholder="Write a message"></textarea>
                           <input type="submit" name="submit" class="myButton mb-5" style="display: block;margin: auto;" >
            </form>
                  </div>
                <h3 class="text-center mt-5">Previous Prescriptions</h3>
        <div class="col-12 col-lg-9 m-auto">
            <?php 
            if(sizeof($previous)==0)echo "<p class='text-center'>None</p>";
            foreach ($previous as $key ) {
                echo'    <div class="p-3 mt-4" style="background-color: #b3cdd1;background-image: linear-gradient(315deg, #b3cdd1 0%, #9fa4c4 74%);"><b>'.$key['problem'].'</b><br>
            '.$key['message'].'
            </div>';}
            ?>
        </div>
