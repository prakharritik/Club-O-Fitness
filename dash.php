...
if (!isset($_SESSION['uid'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM tasks where uid = :prof ');
    $stmt->execute(array(":prof" => $_SESSION['uid']));
...
<head>
  ...
  <title>Health Track</title>
   ...
   <link rel="stylesheet" href="styles.css"><script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js" integrity="sha512-894YE6QWD5I59HgZOGReFYm4dnWc1Qt5NtvYSaNcOP+u1T9qYdvdihz0PPSiiqn/+/3e7Jo4EaG7TubfWGUrMQ==" crossorigin="anonymous"></script><script src='https://kit.fontawesome.com/a076d05399.js' crossorigin='anonymous'></script>
...
.form-detail .textarea{
   ...
   .myButton {
  box-shadow: 11px 17px 23px -6px #e184f3;
...
<body>
  ...
  <h1 class="display-4 mt-5">Hello <?php echo $_SESSION['name'];?></h1>
  <p class="lead">Your BMI is <?php echo $_SESSION['bmi']; ?>
        <?php 
        if($_SESSION['bmi']<18.5){
          echo '<br>Underweight<lottie-player src="https://assets1.lottiefiles.com/temp/lf20_ZGnXlB.json"  background="transparent"  speed="1"  style="width: 100px; height: 100px;display:inline;"  loop  autoplay></lottie-player>';
        }
        else if ($_SESSION['bmi']>=18.5 && $_SESSION['bmi']<=24.9){
          echo '<br>Normal';
        }
        else if ($_SESSION['bmi']>=25.0 && $_SESSION['bmi']<=29.9){
          echo '<br><h3 style="margin-top:-20px;">Overweight</h3><lottie-player src="https://assets1.lottiefiles.com/temp/lf20_ZGnXlB.json"  background="transparent"  speed="1"  style="width: 100px; height: 100px;display:inline;"  loop  autoplay></lottie-player>';
        }
        else echo '<br>Very Overweight<lottie-player src="https://assets1.lottiefiles.com/temp/lf20_ZGnXlB.json"  background="transparent"  speed="1"  style="width: 100px; height: 100px;display:inline;"  loop  autoplay></lottie-player>';
        ?></p>
</div>
<?php
$su=intdiv($_SESSION['tcomp'],5);
if($su>=1)
echo'<div class="ach p-3"><h3 style="font-size:30px;" class="text-center">You have earned '.$su.' <i class="fas fa-award" style="font-size:30px;color:orange"></i></h3></div>';
?>
<h1 style="font-size: 40px;" class="text-center">Your tasks</h1>
<div class="container">
  <?php
  foreach ($rows as $row) {
    echo'<div class="xy row">
      <div class="col-2 d-none d-md-flex">
      <lottie-player src="https://assets6.lottiefiles.com/packages/lf20_v4t3yi.json"  background="transparent"  speed="1"  style="width: 150px; height: 150px;"  loop  autoplay></lottie-player>
      </div>
      <div class="col-12 col-md-8 col-lg-8">
      <div class="card-body">
        <p class="card-text">'.$row['title'].'</p>';
        if($row['perc']==100){
          echo'<lottie-player src="https://assets1.lottiefiles.com/packages/lf20_FizAqM.json"  background="transparent"  speed="1"  style="width: 100px; height: 100px;position:absolute;right:-10px;top:4px;"  autoplay></lottie-player>';
        }
        else if($row['start']==0){echo'<a href="start.php?id='.$row['tid'].'" class="myButton">Start</a>';}
        else {
          echo'<a href="update.php?id='.$row['tid'].'" class="btn btn-primary" style="font-size:15px;">Update</a>';
          echo'<a href="complete.php?id='.$row['tid'].'" class="btn btn-primary ml-5" style="font-size:15px;">Mark Completed</a>';
          echo '<div class="progress mt-3"><div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar"
  aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:'.$row['perc'].'%"> '.$row['perc'].'%</div>
</div>';
        } 
...
<lottie-player src="https://assets2.lottiefiles.com/packages/lf20_6JcVlW.json"  background="transparent"  speed="1"  style="width: 80px; height: 80px;"  loop  autoplay data-toggle="modal" data-target="#myModal" class="adds"></lottie-player>
<div class="modal fade" id="myModal">
  <div class="modal-dialog">
    <div class="modal-content mod">
      <form class="form-detail" action="addtask.php" method="post" id="myform">
                    <h2>Add Task</h2>
                    <div class="form-row">
                      <textarea placeholder="Your Task" name="task"  class="textarea"  required="required"></textarea>
                    </div>
                    <div class="form-row">
                      <input type="number" name="duration" required="required" placeholder="Duration(Days)">
                    </div>                   
...
<lottie-player src="https://assets7.lottiefiles.com/private_files/lf30_P2wyYh.json"  background="transparent"  speed="1"  style="width: 300px; height: 300px;margin: auto;display: block;"  loop  autoplay></lottie-player>
<h1 class="text-center">Task Completed</h1>
    </div>
  </div>
</div>
            <footer style='margin-top: 50px;background-color: #464B6A!important;' class="flex flex-col items-center justify-between px-6 py-2 bg-white dark:bg-gray-800 sm:flex-row" >
    ...
    <?php  if($_SESSION['comp']==1){
      Print "<script>$('#compModal').modal('show'); </script>";
      $_SESSION['comp']=0;
    } ?>
</body>
</html> 