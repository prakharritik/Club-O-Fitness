...
if (!isset($_SESSION['did'])) {
    ...
    $stmt->execute(array(":ii" => $_GET['id']));
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
     $stmt = $pdo->prepare('SELECT * FROM issues i join prescription p on i.iid=p.pid where uid   = :ii ');
    $stmt->execute(array(":ii" => $_GET['id']));
    $previous = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $name=$rows[0]['name'];
    $k=end($rows);
    $file=$k['file'];
    if ( isset($_POST['review']) && strlen($_POST['review'])>=1 ) {
            $stmt = $pdo->prepare('INSERT INTO prescription (message, pid) VALUES (  :fn, :ln)');

        $stmt->execute(array(
                ':fn' => $_POST['review'],
                ':ln' => $k['iid'])
        );
        $stmt = $pdo->prepare('UPDATE issues set  answered=1  where iid=:ln');
...
if ( isset($_POST['refer'])  ) {
                $stmt = $pdo->prepare('UPDATE issues set answered=1 , refer =1 where iid=:ln');
        ...
        $stmt->execute(array(
                ':fn' => 'Our doctor thinks that you must visit some doctor. Physical observation is necessary.',
                ':ln' => $k['iid'])
        );
        header("Location:doctorDash.php");
              return;
  ...
  <nav class="flex items-center justify-between flex-wrap bg-teal p-6">
    ...
    <div  id="menu" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
      ...
      <div>
        ...
        <h1 class="text-center display-4"><?php echo $name;?></h1>
        <div class="col-12 col-lg-9 m-auto">
            <?php if($file!=='0') echo'<img src="'.$file.'" class="d-block m-auto" style="width: 300px;">';?>
        	<div class="p-3 mt-4" style="background-color: #b3cdd1;background-image: linear-gradient(315deg, #b3cdd1 0%, #9fa4c4 74%);">
                <?php echo $k['problem'];?><br>
        	</div>
...
background-image: linear-gradient(315deg, #8ae9b3 0%, #c8d6e5 74%);" rows="2" placeholder="Write a message" required></textarea>
                <input type="submit" name="submit" class=" btn btn-primary " style="display: block;margin: auto;" >
                
            </form>
            <form method="POST"><input type="submit" name="refer" class=" btn btn-primary mt-4" style="display: block;margin: auto;" value="Ask him/her to visit doctor" ></form>
        </div>
